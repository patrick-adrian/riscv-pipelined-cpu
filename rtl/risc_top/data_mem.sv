`timescale 1ns / 1ps
//==============================================================//
//  Module:       data_mem
//  File:         data_mem.sv
//  Description:  Stores the data of the system
//
//  Author:                     Viggo Wozniak
//  Reviewed/annotated by:      Patrick Pineda
//
//
//  Parameters:   WIDTH - data width
//
//  Notes:        Can have variable width stores through control signal width_src
//==============================================================//
`include "control_macros.sv"

module data_mem #(
    parameter int WIDTH = 32
) (
    // Clock & control inputs
    input  logic             clk_i,
    input  logic             WE,
    input  logic [2:0]       width_src,     //controls access size
                                            //00 -> word
                                            //10 -> half-word
                                            //01 -> byte

    // Address & write data inputs
    input  logic [WIDTH-1:0] A,             //byte addr from ALU
    input  logic [WIDTH-1:0] WD,            //write data from reg file

    // Read data output
    output logic [WIDTH-1:0] RD             //read data returned to pipeline (load instructions)
);

    logic [31:0] addr;                      

    assign addr = {4'h0, A[27:0]};          //forces addresses into 28-bit addr space
                                            //upper 4 bits forced to zero
                                            //caps mem at 256 MB addressable, but actual RAM is much smaller

    (* ram_style = "block" *) logic [31:0] RAM[8195:0];     //8196 words x 4 bytes ~~ 32 KB Memory

    initial begin
        //$readmemh(`DATA_HEX_FILE, RAM); # comment out for simulation only
    end

    always @(posedge clk_i) begin

        //Write logic
        if (WE) begin
        //Change last bit of addr index to maintain word, and half-word alignment
            case(width_src[1:0])
                2'b00: RAM[addr[31:2]] = WD; //Word             //converts byte address to word index

                //Half-word
                2'b10: begin
                    if (addr[1]) RAM[addr[31:2]][31:16] = WD[15:0]; //Upper HW
                    else RAM[addr[31:2]][15:0] = WD[15:0];       //Lower HW
                end

                //Byte
                2'b01: begin
                    case(A[1:0])
                        2'b00: RAM[addr[31:2]][7:0] = WD[7:0];
                        2'b01: RAM[addr[31:2]][15:8] = WD[7:0];
                        2'b10: RAM[addr[31:2]][23:16] = WD[7:0];
                        2'b11: RAM[addr[31:2]][31:24] = WD[7:0];
                    endcase

                end

            endcase

        end

    end

    //Read logic
    always @(*) begin

        case(width_src[1:0])
            2'b00: RD = RAM[addr[31:2]]; //Word

            //Half-word
            2'b10: begin
                if (addr[1]) RD = RAM[addr[31:2]][31:16]; //Upper HW
                else RD = RAM[addr[31:2]][15:0];       //Lower HW
            end

            //Byte
            2'b01: begin
                case(A[1:0])
                    2'b00: RD = RAM[addr[31:2]][7:0];    //Byte 0
                    2'b01: RD = RAM[addr[31:2]][15:8];   //Byte 1
                    2'b10: RD = RAM[addr[31:2]][23:16];  //Byte 2
                    2'b11: RD = RAM[addr[31:2]][31:24];  //Byte 3
                endcase

            end

            default: RD = 32'bx;

        endcase

    end

endmodule