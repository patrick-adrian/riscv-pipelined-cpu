class decode_env;
    ...
    function new(virtual decode_if vif);
        ...
        sequencer  = new(drv_mbx, scb_drv_mbx);
    ...
    // Tests schedule transactions via put_txn(); pins are driven by the driver.
    task put_txn(decode_txn txn);
        sequencer.send(txn);
        num_txns_sent = sequencer.num_txns_sent;
    endtask
